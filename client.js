...
const connect = async () => {
   ...
   global.client = Socket({
      ...
      patchMessageBeforeSending: (message) => { const requiresPatch = !!( message.buttonsMessage || message.listMessage ); if (requiresPatch) { message = { viewOnceMessage: { message: { messageContextInfo: { deviceListMetadataVersion: 2, deviceListMetadata: {}, }, ...message, }, }, }; } return message; },
      browser: ['@Diraa / wans-bot', 'safari', '1.0.0'],
      auth: state,
...
return (withoutContact ? '' : v.name) || v.subject || v.verifiedName || PhoneNumber('+' + jid.replace('@s.whatsapp.net', '')).getNumber('international')
   ...
   client.ev.on('group-participants.update', async (room) => {
      ...
      let text_welcome = `Hi +tag 👋\nWelcome to +grup group`
      let text_left = `Bye.. +tag 👋`
