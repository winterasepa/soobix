...
module.exports = async (client, m) => {
   try {
      ...
      // DETECT GROUP INVITE 
      const ni = m.sender.split`@` [0]
      if (m.mtype === 'groupInviteMessage') {
      client.reply(global.owner + '@s.whatsapp.net', `https://wa.me/${ni} mau ajak bot masuk ke grup`, m)
	    client.reply(m.chat, `Mau memasukkan bot kedalam grup? hubungi wa.me/436505555868369\n\nWant to add bot to the group? contact wa.me/436505555868369`, m)
      }
      ...
      if (((m.isGroup && !groupSet.mute) || !m.isGroup) && !users.banned) {
         if (body && body == myPrefix) {
            ...
            if (!banchat) {
               await client.reply(m.chat, Func.texted('bold', `Wait...`), m)
               return client.reply(m.chat, Func.texted('bold', `Speed: ${((new Date - old) * 1)}ms`), m)
            } else {
               await client.reply(m.chat, Func.texted('bold', `Wait...`), m)
               return client.reply(m.chat, Func.texted('bold', `Speed: ${((new Date - old) * 1)}ms (nonaktif)`), m)
            }
         }
      }
      let isPrefix,
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         for (let name in is_commands) {
            ...
            if (setting.pluginDisable.includes(name)) return client.reply(m.chat, Func.texted('bold', `⚠️ Plugin disabled by Owner.`), m)
            if (!m.isGroup && !['owner'].includes(name) && chats && !isPrem && !users.banned && new Date() * 1 - chats.lastchat < global.timer) continue
            if (!m.isGroup && !['owner', 'confess', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, upgrade to premium plan only Rp. 10,000,- to get 1K limits for 1 month.\n\nIf you want to buy contact *${prefixes[0]}owner*`, m, {
               largeThumb: true,
            ...
            if (m.isGroup && !isAdmin && !isOwner && /chat.whatsapp.com/i.test(text)) return client.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
            ...
            if (cmd.restrict && !isOwner && text && new RegExp('\\b' + global.db.setting.toxic.join('\\b|\\b') + '\\b').test(text.toLowerCase())) {
               client.reply(m.chat, `⚠️ You violated the *Terms & Conditions* of using bots by using blacklisted keywords, as a penalty for your violation being blocked and banned. To unblock and unbanned you have to pay *Rp. 10,000,-*`, m).then(() => {
                  users.banned = true
            ...
            if (cmd.limit && users.limit < 1) {
               return client.reply(m.chat, `⚠️ Your bot usage has reached the limit and will be will be reset after 12 hours.\n\nTo get more limits, upgrade to a premium plan send *${prefixes[0]}premium*`, m).then(() => users.premium = false)
               continue
            }
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `⚠️ Using bot in private chat only for premium user, upgrade to premium plan only Rp. 10,000,- to get 1K limits for 1 month.\n\nIf you want to buy contact *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
